    ...
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[110] flex items-center justify-center p-4 animate-fade-in" onClick={onClose}>
      <div className="glass-card max-w-md w-full !p-8 animate-scale-in relative overflow-hidden" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between mb-8">
          <h2 className="text-2xl font-semibold text-white">Trust Calibration</h2>
        ...
        <div className="mb-8 p-4 bg-white/[0.02] border border-white/5 rounded-2xl">
          <p className="text-[10px] font-semibold text-slate-500 uppercase tracking-widest mb-1">Target Entity</p>
          <p className="text-white font-semibold truncate">{trademarkName}</p>
          <p className="text-[10px] font-mono text-slate-600 truncate mt-1">Node: {creatorAddress}</p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-8">
          <div className="flex flex-col items-center">
            <label className="text-[10px] font-semibold text-slate-500 uppercase tracking-widest mb-4">Sentiment Score</label>
          ...
          <div>
            <label className="text-[10px] font-semibold text-slate-500 uppercase tracking-widest ml-1 mb-2 block">Optional Narrative</label>
            <textarea
