    ...
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-[100] p-4 animate-fade-in">
      <div className="glass-card max-w-md w-full !p-8 relative overflow-hidden animate-scale-in">
        ...
        <div className="flex items-center justify-between mb-8 relative z-10">
          <h2 className="text-2xl font-black text-white">Issue License</h2>
        ...
        <div className="mb-8 p-4 bg-indigo-500/5 rounded-2xl border border-indigo-500/10">
          ...
          <p className="text-white font-black">{trademarkName}</p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-6 relative z-10">
          <div>
            <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1 mb-2 block">Price per period (ETH)</label>
            <input
          ...
          <div>
            <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1 mb-2 block">License Vector (Duration)</label>
            <div className="grid grid-cols-2 gap-3">
              {LICENSE_DURATIONS.map((opt) => (
                <button
                  key={opt.label} type="button"
                  onClick={() => setDuration(opt.value)}
                  className={`px-4 py-3 rounded-xl border-2 text-xs font-black transition-all ${
                    duration === opt.value ? 'bg-indigo-600 border-indigo-600 text-white shadow-lg shadow-indigo-500/20' : 'bg-white/5 border-white/5 text-slate-400 hover:border-white/10'
                  }`}
                >
                  {opt.label}
                </button>
