...
const PartnerLogo = ({ name, icon }: { name: string; icon?: string }) => (
  <div className="flex items-center gap-3 opacity-40 hover:opacity-100 transition-all grayscale hover:grayscale-0 cursor-default px-8 group">
    <div className="w-6 h-6 bg-slate-900/10 dark:bg-white/10 rounded-md transition-colors group-hover:bg-indigo-500/20" />
    <span className="text-sm font-bold tracking-tighter text-slate-900 dark:text-white">{name}</span>
  </div>
);
...
export default function Home() {
  ...
  return (
    <AnimatedPage>
      <div className="selection:bg-indigo-500/30 min-h-screen relative overflow-hidden flex flex-col transition-colors duration-500">
        <Head>
        ...
        <main className="relative pt-20 flex-1">
          ...
          {/* Hero Section */}
          <section className="relative min-h-[85vh] flex flex-col items-center justify-center text-center px-6 -translate-y-[4%]">
            {/* Floating Nodes */}
            ...
            <motion.div
              ...
              className="mb-5"
            >
              <Link href="/register" className="inline-flex items-center gap-2 px-4 py-1.5 bg-slate-900/5 dark:bg-white/5 border border-slate-900/10 dark:border-white/20 rounded-full text-[10px] font-bold text-slate-600 dark:text-white/70 hover:bg-slate-900/10 dark:hover:bg-white/10 transition-colors group tracking-widest uppercase shadow-sm">
            ...
            <motion.h1
              ...
              className="text-4xl md:text-7xl font-bold tracking-tight mb-8 max-w-5xl leading-[1.1] text-slate-900 dark:text-white"
            >
              The Sovereign Layer for <br /><span className="text-slate-900/50 dark:text-white/40 ">Global Brand Integrity</span>
            </motion.h1>
            ...
            <motion.p
              ...
              className="text-sm md:text-lg text-slate-600 dark:text-white/60 max-w-2xl mb-12 leading-relaxed font-semibold italic"
            >
              Establishing an institutional source of truth for your intellectual property. <br className="hidden md:block" />
              Secure, verify, and license your trademarks on the global ledger.
            </motion.p>
          ...
          {/* Partner Logos Bar */}
          <section className="py-20 border-t flex flex-col items-center">
            <div className="w-full overflow-hidden relative">
              ...
              {/* Fade overlays */}
              <div className="absolute inset-y-0 left-0 w-40 bg-gradient-to-r from-[var(--bg-primary)] to-transparent z-10" />
              <div className="absolute inset-y-0 right-0 w-40 bg-gradient-to-l from-[var(--bg-primary)] to-transparent z-10" />
            </div>
          </section>

          {/* Core Features Integration */}
          <section id="features" className="py-32 container-custom relative z-10 border-t transition-colors duration-500">
            <div className="grid md:grid-cols-3 gap-12">
              {[
                ...
                <motion.div
                  ...
                  className="group p-8 rounded-3xl glass-card transition-all"
                >
                  <div className="w-10 h-10 bg-indigo-500/10 dark:bg-indigo-500/20 rounded-xl flex items-center justify-center mb-6 text-indigo-600 dark:text-indigo-400 font-bold group-hover:scale-110 transition-transform">
                    0{i + 1}
                  </div>
                  <h3 className="text-xl font-bold mb-3">{f.title}</h3>
                  <p className="text-sm text-slate-600 dark:text-white/50 leading-relaxed font-semibold">{f.desc}</p>
                </motion.div>
              ))}
            </div>
          </section>
